...
describe('useAudioRecorder — zéro perte', () => {
  ...
  it("l'audio part intact : ce qui a été capté est exactement ce qui est envoyé", async () => {
    const h = fabriquer();
    const recorder = await demarrer(h);
    act(() => {
      recorder.emit('ABC');
    ...
    await waitFor(() => expect(h.result.current.status).toBe('done'));
    const envoi = (h.upload as unknown as { mock: { calls: Array<[{ blob: Blob }]> } }).mock
      .calls[0]?.[0];
    expect(await envoi?.blob.text()).toBe('ABC');
    expect(h.createRow).toHaveBeenCalledWith(expect.objectContaining({ sizeBytes: 3 }));
  });
