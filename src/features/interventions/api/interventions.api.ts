 ...
 *
 * La RPC PostgreSQL ferme l'éventuel segment actif du compte — même sur une
 * autre intervention ou organisation — puis ouvre le nouveau dans une seule
 * transaction. Deux onglets ne peuvent donc ni doubler ni perdre le chrono.
 */
export async function switchTimeEntry(input: {
  ...
  const { data, error } = await supabase.rpc('switch_intervention_time_entry', {
 ...
 */
export async function listReportsPendingReview(organizationId: string): Promise<ReportForReview[]> {
  return unwrap(
    supabase
      .from('intervention_reports')
      .select(
...
export async function countReportsByStatus(
  ...
  const rows = await unwrap(
    supabase.from('intervention_reports').select('status').eq('organization_id', organizationId),
  );
