...
export function DevRoleSelector() {
  ...
  return (
    <div className="flex items-center gap-1.5 rounded-xl border border-amber-500/30 bg-amber-500/10 px-2.5 py-1 text-2xs font-medium text-amber-300 dark:bg-amber-500/10 dark:text-amber-400">
      <Sparkles className="size-3.5 text-amber-400 animate-pulse" />
      <span className="hidden md:inline font-bold uppercase tracking-wider text-3xs text-amber-400">
        DEV ROLE :
      </span>
      ...
      <select
        ...
        className="bg-transparent font-semibold text-white focus:outline-none cursor-pointer rounded px-1 text-xs"
        aria-label="Sélectionner le rôle simulé en développement"
      >
        <option value="entrepreneur" className="bg-slate-900 text-white">
          👔 Entrepreneur (Owner)
        </option>
        <option value="technician" className="bg-slate-900 text-white">
          👷 Technicien (Terrain)
        </option>
      </select>
    </div>
