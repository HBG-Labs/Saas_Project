...
import {
  createPortalRequestAccessHandler,
  renderOtpEmail,
  type AccessStore,
  type PortalGate,
} from './handler.ts';
...
function setup(
  ...
  const store: AccessStore = {
    ...
    claimRequest: (gate) => {
      if (options.quotaGranted === false) return Promise.resolve(false);
      journal.recorded.push(gate);
      return Promise.resolve(true);
    },
    issueOtp: (email) => {
      journal.issued.push(email);
      return Promise.resolve({ code: '482913' });
    },
  };
...
Deno.test(
  'AC04 — un contact autorisé reçoit un code par courriel, jamais dans la réponse',
  async () => {
...
Deno.test(
  'AC04/AC05 — une adresse inconnue ou révoquée obtient la même réponse, sans code',
  async () => {
...
Deno.test('au-delà du plafond, la réponse ne change pas mais rien ne part', async () => {
  const { handler, journal } = setup({ quotaGranted: false });
  const res = await handler(post({ email: 'jean.client@example.com' }));
  assertEquals(res.status, 200);
...
Deno.test('sans configuration, la fonction refuse sans exposer les secrets attendus', async () => {
  const { handler, journal } = setup({ missing: ['RESEND_API_KEY', 'PORTAL_FROM_EMAIL'] });
  const res = await handler(post({ email: 'jean.client@example.com' }));
  assertEquals(res.status, 503);
  assertEquals(
    ((await res.json()) as { error: string }).error.includes('PORTAL_FROM_EMAIL'),
    false,
  );
  assertEquals(journal.issued, []);
});
