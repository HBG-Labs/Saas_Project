...
export function createQuotePdfHandler(config: QuotePdfServiceConfig) {
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      if (authError || !auth.user) return json({ error: 'Votre session a expiré. Reconnectez-vous.' }, 401);
      ...
      const { data: quote, error: quoteError } = await caller
        ...
        .eq('id', quoteId)
        .maybeSingle();
      if (quoteError) return json({ error: 'Le devis ne peut pas être chargé pour le moment.' }, 503);
      ...
      async function ready(doc: { object_path: string; pdf_sha256: string; byte_size: number; generated_at: string }) {
        const { data, error } = await caller.storage.from(BUCKET).createSignedUrl(doc.object_path, 60);
        if (error || !data) {
          return json({ error: 'Le document conservé est temporairement indisponible. Réessayez.' }, 503);
        }
        return json({ url: data.signedUrl, sha256: doc.pdf_sha256, byteSize: doc.byte_size, generatedAt: doc.generated_at });
      }

      const { data: existing, error: readError } = await table.select('*').eq('quote_id', quoteId).maybeSingle();
      if (readError) return json({ error: 'La conservation des documents est temporairement indisponible.' }, 503);
      if (existing) return await ready(existing);

      const [itemsResult, organizationResult] = await Promise.all([
        caller.from('quote_items').select('*').eq('quote_id', quoteId).order('position').order('id').limit(501),
      ...
      if (!organizationResult.data) return json({ error: 'Entreprise introuvable ou inaccessible.' }, 404);
      ...
      const subtotalCents = items.reduce((sum, it) => sum + it.line_total_cents, 0);
      ...
      const input: QuotePdfInput = {
        ...
        payment_method: (org.quote_payment_method as string | null)?.trim() || DEFAULT_PAYMENT_METHOD,
        items,
      };

      let pdf: Uint8Array;
      try {
        pdf = await renderQuotePdf(PDFDocument, input, organization, config.now?.() ?? new Date());
      } catch (error) {
        console.error('quote pdf render failed', error instanceof Error ? error.name : 'unknown');
        return json({ error: error instanceof Error ? error.message : 'Le PDF n’a pas pu être préparé.' }, 422);
      }
      ...
      const { error: uploadError } = await admin.storage
        .from(BUCKET)
        .upload(objectPath, pdf, { contentType: 'application/pdf', upsert: false, cacheControl: '0' });
      if (uploadError) {
        ...
        const { data: stored, error: downloadError } = await admin.storage.from(BUCKET).download(objectPath);
        if (
          downloadError ||
          !stored ||
          ...
          return json(
            { error: 'Le document n’a pas pu être conservé. Réessayez ; aucun fichier existant n’a été remplacé.' },
            503,
          );
        }
      }
      ...
      const { data: inserted, error: insertError } = await table.insert(document).select('*').single();
      if (insertError) {
        const { data: concurrent } = await table.select('*').eq('quote_id', quoteId).maybeSingle();
        if (concurrent) return await ready(concurrent);
        return json({ error: 'Le document est en cours de conservation. Réessayez dans quelques instants.' }, 503);
      }
