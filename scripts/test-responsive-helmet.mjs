...
async function testResponsiveHelmet() {
  const browser = await chromium.launch({ headless: true });
  
  const viewports = [
    { name: 'desktop', width: 1440, height: 950, shift: 380 },
    { name: 'laptop', width: 1024, height: 800, shift: 280 },
    { name: 'tablet', width: 768, height: 1024, shift: 180 },
    { name: 'mobile', width: 390, height: 844, shift: 100 }
  ];
  ...
  for (const vp of viewports) {
    ...
    const outPath = path.join(artifactsDir, `helmet_responsive_${vp.name}.png`);
...
testResponsiveHelmet().catch(console.error);
