...
async function testBentoShifts() {
  ...
  const shifts = [180, 240, 300, 360, 420, 480];
  ...
  for (const s of shifts) {
    await page.evaluate((val) => {
      ...
      if (img) {
        ...
        img.setAttribute('style', `position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; object-position: center; transform: translateY(-${val}px); opacity: 0.45; filter: saturate(1.1) contrast(1.05);`);
      }
    }, s);
    ...
    if (bentoSection) {
      ...
      const outPath = path.join(artifactsDir, `bento_shift_${s}px.png`);
      await bentoSection.screenshot({ path: outPath });
      console.log(`Saved bento shift ${s}px to:`, outPath);
    }
  }
...
testBentoShifts().catch(console.error);
